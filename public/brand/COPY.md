# X copy — Witchcat

Display name: Witchcat
Handle: WitchcatLOL
Location: Robinhood Chain
Website: https://www.witchcat.lol

Bio (106 / 160):

Find the cats. Change the seasons. Every swap feeds the cauldron. $WITCH on Robinhood Chain · witchcat.lol

Images in /x:

- logo.png — 400×400 profile (witch, use this)
- logo-cover.png — 400×400 alt (full cover)
- banner.png — 1500×500 header (Play / Seasons / Cauldron, site type)
- post.png — 1500×1000 pin / live still
- article.png — 1500×600

Pin (attach post.png):

The cats vanished.

A witch, four seasons, twelve familiars.
Play it. No stamp. No keeper.

https://www.witchcat.lol

Unofficial token. Game by Satanimax, pixel art by Lylouf. MIT.

Live (attach post.png, paste CA):

$WITCH is live.

Find the cats. Change the seasons.
Every swap feeds the cauldron.

https://www.witchcat.lol
CA

Seasons (attach article.png):

Winter freezes the water.
Autumn fills the pits.
Summer grows the vines.
Spring burns the stone.

Twelve cats. Four temples.

https://www.witchcat.lol/play

Reply:

Unofficial. Not JS13K. Game: Witch Cat by Satanimax. Pixel art: Lylouf. MIT. $WITCH is original.
